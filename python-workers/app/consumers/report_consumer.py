...
async def process(message: AbstractIncomingMessage) -> None:
    async with message.process(requeue=False):
        ...
        try:
            ...
            # Rapor event'leri daha kısa TTL — 1 saat
            if cid and await _guard.is_processed(cid, ttl_seconds=3600):
                messages_processed.labels(queue=QUEUE, status="skipped").inc()
                return

            await report_handler.handle(payload)
            ...
            logger.error("report_consumer_error", error=str(exc),
