...
from app.handlers import audit_handler
...
async def process(message: AbstractIncomingMessage) -> None:
    async with message.process(requeue=False):
        ...
        try:
            ...
            if cid and await _guard.is_processed(cid, ttl_seconds=3 * 24 * 3600):
                messages_processed.labels(queue=QUEUE, status="skipped").inc()
                return

            await audit_handler.handle(payload)
            ...
            logger.error("audit_consumer_error", error=str(exc),
