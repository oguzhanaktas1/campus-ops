...
export default function AdminAccessRequestsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${BACKEND}/admin/access-requests`, {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 pb-20">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Review all campus system and physical access requests.</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search by resource, requester, type..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 border-b border-border">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground">Access Type</th>
                  <th className="text-left px-4 py-3 font-medium text-muted-foreground">Target Resource</th>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 font-medium">{r.requester?.profile?.fullName ?? '-'}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.accessType}</td>
                    <td className="px-4 py-3 max-w-[180px] truncate">{r.targetResource}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.startAt ? fmt(r.startAt) : '-'}</td>
                    <td className="px-4 py-3 text-muted-foreground">{r.endAt ? fmt(r.endAt) : '-'}</td>
