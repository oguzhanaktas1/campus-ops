    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto pb-20">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            placeholder="Search by URL or integration..."
            className="pl-9"
            value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden sm:table-cell">Integration</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden md:table-cell">Method</th>
                  ...
                  {expandedId === l.id && (l.payload || l.response) && (
                    <tr key={`${l.id}-detail`} className="bg-muted/30">
                      <td colSpan={7} className="px-5 py-3 space-y-2">
                        {l.payload && (
                          <div>
                            <p className="text-xs font-semibold text-muted-foreground mb-1">Request Payload</p>
                        ...
                        {l.retryCount > 0 && (
                          <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">Retried {l.retryCount} time{l.retryCount > 1 ? 's' : ''}</p>
                        )}
                      </td>
                    </tr>
                  )}
