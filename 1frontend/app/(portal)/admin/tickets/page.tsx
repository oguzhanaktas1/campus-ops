...
const STATUS_BADGE: Record<string, string> = {
  ...
  CLOSED:
    'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800/30 dark:text-slate-400 dark:border-slate-700',
}
...
export default function AdminTicketsPage() {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      ...
      setTickets(
        raw.map((t) => ({
          id: t.request?.id ?? t.id,
          requestNo: t.request?.requestNo ?? '-',
          category: t.category ?? '-',
          priority: t.request?.priority ?? 'MEDIUM',
          status: t.ticketStatus ?? 'OPEN',
          reportedBy: t.reportedBy?.profile?.fullName ?? '-',
          assignedTo: t.assignedTo?.profile?.fullName ?? null,
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto pb-20">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Platform-wide IT support ticket overview.
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden md:table-cell">
                  Reporter
                </th>
                ...
                <th className="px-5 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden xl:table-cell">
                  Created
                </th>
              </tr>
            </thead>
                  ...
                  <td className="px-5 py-3.5">
                    <span
                      className={cn(
                        ...
                        PRIORITY_BADGE[t.priority] ?? PRIORITY_BADGE.LOW,
                      )}
                    >
                      {t.priority}
                    </span>
