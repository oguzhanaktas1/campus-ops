      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-background shadow-2xl">
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-foreground">New Integration</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4 p-5">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
              Key <span className="normal-case text-[11px] text-muted-foreground">(auto-generated if empty)</span>
            </label>
            <Input
              placeholder="e.g. SLACK_NOTIFICATIONS"
              value={form.key}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
              Webhook URL <span className="normal-case text-[11px] text-muted-foreground">(optional)</span>
            </label>
            <Input
              type="url"
...
export default function AdminIntegrationsPage() {
  ...
  const handleSync = async (id: string, name: string) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(`"${name}" synced successfully (${data.responseTime}ms).`)
      } else {
        toast.error(`"${name}" sync failed: ${data.error ?? `HTTP ${data.statusCode}`}`)
      }
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-6 pb-20">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage external service connections and test webhook delivery.
          </p>
        </div>
      ...
      {integrations.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card py-20 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Add your first integration to get started.</p>
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={isBusy}
                    title={int.isActive ? 'Disable' : 'Enable'}
                  >
                    {isToggling
                      ? <Loader2 className="size-3 animate-spin" />
                      : int.isActive
