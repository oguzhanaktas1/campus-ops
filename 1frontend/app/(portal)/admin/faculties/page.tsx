...
export default function FacultiesPage() {
  ...
  const openEdit = (faculty: Faculty) => {
    ...
    setForm({ name: faculty.name, code: faculty.code, campusId: faculty.campusId, isActive: faculty.isActive })
    setDialogOpen(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editingItem
        ? `${backendUrl}/admin/faculties/${editingItem.id}`
        : `${backendUrl}/admin/faculties`
  ...
  const filtered = faculties.filter(
    (f) =>
      ...
      f.campusName?.toLowerCase().includes(search.toLowerCase()),
  )
        ...
        <table className="w-full text-sm min-w-[600px]">
          ...
          <tbody className="divide-y divide-border">
            {filtered.map((faculty) => (
              <tr key={faculty.id} className="hover:bg-muted/20 transition-colors">
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-3">
                    <div className="size-8 rounded-md bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-700 dark:text-emerald-400 flex-shrink-0">
                ...
                <td className="px-5 py-3.5 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" size="icon" className="size-7" onClick={() => openEdit(faculty)}>
