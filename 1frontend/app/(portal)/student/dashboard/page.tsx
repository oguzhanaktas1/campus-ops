...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        // 🔥 Tüm verileri aynı anda, beklemeden asenkron çeker 🔥
        const [userRes, reqRes, aptRes, resRes, notifRes] = await Promise.all([
  ...
  // 🔥 GERÇEK VERİLERİ UI'A GÖRE FİLTRELEME VE HAZIRLAMA 🔥
  const open = requests.filter(r => !['COMPLETED', 'APPROVED', 'REJECTED', 'CANCELLED', 'CLOSED', 'EXPIRED', 'RESOLVED'].includes(r.status))
  const pendingApprovals = requests.filter(r => ['SUBMITTED', 'IN_REVIEW', 'WAITING_APPROVAL'].includes(r.status))
  const completed = requests.filter(r => ['COMPLETED', 'APPROVED', 'RESOLVED'].includes(r.status))
  
  const upcomingApts = appointments.filter(a => ['requested', 'confirmed'].includes(a.status?.toLowerCase()))
  const confirmedRes = reservations.filter(r => ['pending', 'approved', 'active'].includes(r.status?.toLowerCase()))
  
  // UI'ın beklediği renk yapılarına uyması için backend Notification type'larını çeviriyoruz
  const unread = notifications.filter(n => !n.isRead).slice(0, 3).map(n => ({
    ...n,
    uiType: n.type === 'SYSTEM' ? 'warning' : 'info' // Backend'deki IN_APP default info (mavi) olacak
  }))
  ...
  // AŞAĞIDAKİ JSX YAPISINA VE CLASSLARA KESİNLİKLE DOKUNULMADI
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      {/* Greeting */}
      <div>
        <h1 className="text-xl font-bold text-foreground">Good morning, {firstName}</h1>
        <p className="text-sm text-muted-foreground mt-0.5">Here's a summary of your campus activity.</p>
      </div>
            ...
            return (
              <Link key={action.href} href={action.href}>
                <div className="bg-card border border-border rounded-lg p-4 flex flex-col items-center gap-2 hover:shadow-md transition-shadow cursor-pointer text-center">
                  <div className={`size-10 rounded-lg flex items-center justify-center ${action.color}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-lg shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            <h2 className="text-sm font-semibold text-foreground">My Open Requests</h2>
          ...
          <div className="divide-y divide-border">
            {open.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">No open requests.</p>
            ) : (
                ...
                <Link key={req.id} href={`/student/requests/${req.id}`}>
                  <div className="px-5 py-3.5 flex items-start justify-between gap-3 hover:bg-muted/30 transition-colors cursor-pointer">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 capitalize">{req.type?.replace(/_/g, ' ')} · {formatDate(req.createdAt)}</p>
                    </div>
            ...
            <div className="divide-y divide-border">
              {upcomingApts.slice(0, 2).map((apt) => (
                <div key={apt.id} className="px-4 py-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{apt.with}</p>
                  <p className="text-xs text-muted-foreground">{formatDate(apt.date)} · {apt.time} · {apt.location}</p>
                </div>
              ))}
            ...
            <div className="divide-y divide-border">
              {unread.map((n) => (
                <div key={n.id} className="px-4 py-3 flex gap-2.5 items-start">
                  <div className={`size-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                    n.uiType === 'success' ? 'bg-emerald-500' :
                    n.uiType === 'warning' ? 'bg-amber-500' :
                    n.uiType === 'error' ? 'bg-red-500' : 'bg-blue-500'
                  }`} />
