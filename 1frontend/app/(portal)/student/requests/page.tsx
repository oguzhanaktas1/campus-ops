...
import { Clock, PlusCircle, Loader2, MessageSquare, AlertCircle, User } from 'lucide-react'
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-5 pb-20">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {requests.map((req) => {
            // 🔥 AKILLI ATAMA MANTIĞI: İkisinden biri doluysa onu al, eğer ikisi de varsa currentAssignee'yi (yani son güncel olanı) önceliklendir.
                ...
                <div className="bg-card border border-border rounded-lg p-5 shadow-sm hover:border-primary/50 transition-colors">
                  <div className="flex items-start justify-between gap-2 flex-wrap mb-2">
                    ...
                    <div className="flex gap-2">
                      <Badge variant="outline" className={req.priority === 'HIGH' || req.priority === 'URGENT' ? 'border-red-200 text-red-600 bg-red-50' : 'bg-muted'}>
                        {req.priority}
                      </Badge>
                      <Badge variant="outline" className={statusColor[req.status.toLowerCase()] || statusColor['pending']}>
                  ...
                  <div className="flex items-center gap-4 flex-wrap mt-3">
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Clock className="size-3.5" /> {new Date(req.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                    ...
                    {/* 🔥 TEK VE AKILLI ATAMA GÖSTERGESİ 🔥 */}
                    {assigneeToShow && (
                      <span className="flex items-center gap-1 text-xs text-muted-foreground border-l border-border pl-4">
                        <User className="size-3.5" /> Assigned to: <span className="font-medium text-foreground">{assigneeToShow}</span>
                      </span>
                    )}
          ...
          {requests.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center bg-card border border-dashed rounded-lg">
              ...
              <p className="text-xs text-muted-foreground mt-1 mb-4">You haven't created any {pageTitle.toLowerCase()} yet.</p>
            </div>
          )}
        </div>
      )}
