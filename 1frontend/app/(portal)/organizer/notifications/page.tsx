...
export default function OrganizerNotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await fetch(`${backendUrl}/organizer/notifications`, {
  ...
  const handleNotificationClick = async (notif: any) => {
    try {
      if (!notif.isRead) {
        const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'
        await fetch(`${backendUrl}/organizer/notifications/${notif.id}/read`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${getToken()}` }
        })
  ...
  const handleMarkAllRead = async () => {
    try {
      const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'
      const res = await fetch(`${backendUrl}/organizer/notifications/read-all`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` }
      })
