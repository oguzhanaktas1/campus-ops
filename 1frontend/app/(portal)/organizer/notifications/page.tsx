...
export default function OrganizerNotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch(`${BACKEND}/organizer/notifications`, {
  ...
  const handleNotificationClick = async (notif: any) => {
    try {
      if (!notif.isRead) {
        await fetch(`${BACKEND}/organizer/notifications/${notif.id}/read`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${getToken()}` },
        })
  ...
  const handleMarkAllRead = async () => {
    try {
      const res = await fetch(`${BACKEND}/organizer/notifications/read-all`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      if (res.ok) {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-foreground">{t('common.notifications')}</h1>
