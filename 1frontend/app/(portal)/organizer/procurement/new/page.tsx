...
export default function OrganizerNewProcurementPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.itemName.trim()) {
      toast.error(t('pages.itemNameRequired'))
      return
    }

    if (!form.justification.trim()) {
      toast.error(t('pages.justificationRequired'))
      return
    }

    const quantity = Number(form.quantity)
    if (!Number.isInteger(quantity) || quantity < 1) {
      toast.error(t('pages.quantityMin'))
      return
    }
    ...
    if (
      ...
      toast.error(t('pages.unitPriceInvalid'))
      return
    }
    ...
    try {
      ...
      toast.success(t('pages.procurementSubmitted', { requestNo: data.requestNo }))
      router.push(`/organizer/requests/${data.requestId}`)
    } catch (error: any) {
      toast.error(error.message || t('pages.somethingWrong'))
    } finally {
      ...
      >
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-1.5 sm:col-span-2">
            <Label>
              {t('pages.itemName')} <span className="text-destructive">*</span>
            </Label>
            <Input
              value={form.itemName}
              onChange={(e) => setValue('itemName', e.target.value)}
              placeholder={t('pages.itemPlaceholder')}
            />
          </div>
          ...
          <div className="space-y-1.5">
            <Label>
              {t('common.quantity')} <span className="text-destructive">*</span>
            </Label>
            <Input
              type="number"
              min="1"
          ...
          <div className="space-y-1.5">
            <Label>{t('pages.unitPriceEstimate')}</Label>
            <Input
              type="number"
              min="0"
              step="0.01"
              ...
              placeholder={t('pages.unitPricePlaceholder')}
            />
          </div>

          <div className="space-y-1.5">
            <Label>{t('pages.vendorPreference')}</Label>
            <Input
              value={form.vendorPreference}
              onChange={(e) => setValue('vendorPreference', e.target.value)}
              placeholder={t('pages.vendorPlaceholder')}
            />
          </div>

          <div className="space-y-1.5">
            <Label>{t('pages.budgetCode')}</Label>
            <Input
              value={form.budgetCode}
              onChange={(e) => setValue('budgetCode', e.target.value)}
              placeholder={t('pages.budgetPlaceholder')}
            />
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <Label>
            {t('pages.justification')} <span className="text-destructive">*</span>
          </Label>
          <textarea
            rows={5}
            value={form.justification}
            onChange={(e) => setValue('justification', e.target.value)}
            placeholder={t('pages.purchaseJustificationPlaceholder')}
