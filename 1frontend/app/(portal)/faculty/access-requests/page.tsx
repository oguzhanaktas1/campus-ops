...
import { ShieldCheck, Loader2, Search, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function FacultyAccessRequestsPage() {
  const { t } = useI18n()
  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setRequests(data.filter((r: any) => r.type === 'ACCESS_REQUEST'))
      } else {
        setRequests([])
      }
    } catch {
      toast.error(t('accessRequests.loadFail'))
      setRequests([])
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5 pb-20">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          <h1 className="text-xl font-bold text-foreground">{t('accessRequests.title')}</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {t('accessRequests.subtitle', { count: requests.length })}
          </p>
        </div>
      ...
      <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-foreground">{t('accessRequests.noRequests')}</p>
          </div>
        ) : (
