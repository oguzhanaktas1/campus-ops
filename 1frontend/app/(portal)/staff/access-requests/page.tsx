  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5 pb-20">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          <h1 className="text-xl font-bold text-foreground">{t('pages.accessRequests')}</h1>
      ...
      <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-foreground">{t('pages.noAccessRequests')}</p>
          </div>
        ) : (
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="text-sm font-semibold text-foreground truncate">
                      {r.targetResource || 'Access Request'}
                    </p>
                    {r.requestNo && (
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                    ...
                    {r.accessType && <span>{r.accessType}</span>}
                    {r.justification && (
                      <span className="italic">&ldquo;{r.justification.slice(0, 60)}&hellip;&rdquo;</span>
                    )}
                  </div>
                </div>
