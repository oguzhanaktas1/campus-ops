              ...
              {['SUBMITTED', 'IN_REVIEW'].includes(ev.status) && (
                actionId === ev.id ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <Button size="sm" variant="outline" onClick={() => updateStatus(ev.id, 'IN_REVIEW')}>Mark In Review</Button>
                  ...
                  <Button size="sm" variant="outline" onClick={() => setActionId(ev.id)}>Process</Button>
                )
              )}
            </div>
          ))
