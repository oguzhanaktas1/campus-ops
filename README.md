...
- Production `.env` dosyalarını oluşturun.
- Secret değerlerini rotate edin.
- Veritabanı migration'larını çalıştırın.
- Seed komutlarını sadece gerekiyorsa ve production'a uygun seed ile çalıştırın.
- Reverse proxy, TLS, firewall ve volume backup stratejisini kurun.
- RabbitMQ management portunu public internete doğrudan açmayın.
- Postgres ve Redis portlarını sadece güvenilir ağdan erişilebilir yapın.

## Açık Kaynak Yayına Hazırlık

Repo şu anda ürün geliştirme monorepo'su olarak duruyor. Açık kaynak yayın için aşağıdaki işleri tamamlamak gerekir:

- `LICENSE` dosyası ekleyin. MIT veya Apache-2.0 gibi bir lisans seçin.
- `CONTRIBUTING.md` ekleyin.
- `SECURITY.md` ekleyin.
- `.env` dosyalarını repodan çıkarın ve tüm secret değerlerini rotate edin.
- Her servis için `.env.example` dosyası oluşturun.
- `.gitignore` dosyasını genişletin:
  - `node_modules`
  - `.next`
  - `dist`
  - `.env`
  ...
  - `__pycache__`
  - `*.tsbuildinfo`
  - Docker/local volume çıktıları
- Demo seed kullanıcılarını ve demo şifrelerini açıkça dokümante edin.
- Third-party dependency lisanslarını kontrol edin.
- Üretim domain, key, token veya kurum içi veri içeren dosyaları temizleyin.
- CI workflow ekleyin:
  - Frontend build/lint
  - Backend build/test
  - Prisma generate/migration check
  - Python lint/test, eğer test altyapısı eklenirse

## İlgili Dokümanlar

Repo içinde mimari ve workflow detaylarını genişleten ek belgeler vardır:

- `CampusOps_AI_Assistant_Architecture_Guide.md`
- `campus_ops_unified_workflow.md`
- `campus_ops_workflow.md`
- `Sentetik-veri-ve-log-uretimi.md`

Bu README, projenin çalıştırılması ve genel mimarisi için ana giriş noktasıdır. Domain veya workflow davranışı değiştirildiğinde ilgili teknik dokümanlarla birlikte bu dosya da güncellenmelidir.
